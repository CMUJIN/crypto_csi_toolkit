...
-------------------------------------------------
✅ 保留原始筹码分析逻辑与输出文件命名：
    - docs/{SYMBOL}_chip_strength.csv
    - docs/{SYMBOL}_chip_timeline_pro.png
✅ 新增流动性曲线（严格使用 --liquidity_start 截取数据）：
    - 左轴：Price（红色）
    - 右轴：Liquidity Z-Score（蓝色）
    - 右轴水平虚线：-2, -1, 0, 1, 2
    - 上图：全样本周线；下图：最近90天日线
    - 输出：docs/{SYMBOL}_liquidity_{YYYY-MM-DD}.png
✅ 与 upload_to_notion.py 的扫描/展示规则完全兼容
"""
...
# =========================================================
# Liquidity helpers (match your confirmed logic)
# =========================================================
...
def main():
    ...
    chip_png = Path(args.out_dir) / f"{symbol}_chip_timeline_pro.png"
    ...
    if args.liquidity_start:
        ...
        liq_png = Path(args.out_dir) / f"{symbol}_liquidity_{liq_start.strftime('%Y-%m-%d')}.png"
